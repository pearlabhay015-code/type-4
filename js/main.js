...
function renderHomepageStats(data) {
  ...
  const admissions = stats.yearlyAdmissions || stats.monthlyAdmissions || homepageStatsFallback.yearlyAdmissions;
  ...
  container.innerHTML = `
    ...
    <div class="homepage-dashboard-grid">
      ...
      <article class="homepage-chart-card">
        ...
        <div class="admissions-bars" role="img" aria-label="${escapeStatHtml(admissions.title)} bar chart">${admissionData.map(item => `<div class="admissions-bar-group"><div class="admissions-bars-pair"><i style="height:${(Number(item.primary) || 0) / admissionMax * 100}%" title="Admitted: ${item.primary}"></i><i style="height:${(Number(item.secondary) || 0) / admissionMax * 100}%" title="Applications: ${item.secondary}"></i></div><span>${escapeStatHtml(item.label)}</span></div>`).join('')}</div>
      </article>
